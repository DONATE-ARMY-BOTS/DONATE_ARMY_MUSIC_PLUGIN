...
async def banall(client: Client, message: Message):
    ...
    AMBOTOK = await message.reply_text(
        f"Hey {message.from_user.mention}, 'deleteall' checking..."
    )
    await asyncio.sleep(2)
    bot = await app.get_chat_member(chat_id, app.me.id)
    if not (
        bot.privileges.can_delete_messages
        and bot.privileges.can_promote_members
        and bot.privileges.can_invite_users
    ):
        await AMBOTOK.edit(
...
async def handle_callback(client: Client, callback_query: CallbackQuery):
    ...
    if callback_query.data == "deleteall_yes":
        ...
        try:
            ...
            await callback_query.answer(
                "All messages deleted successfully.", show_alert=False
            )
        except Exception as e:
            await callback_query.answer(
                f"An error occurred: {str(e)}", show_alert=False
            )
...
async def ban_all(client: Client, message: Message):
    ...
    async for member in client.get_chat_members(chat_id):
        ...
        ] or member.user.id in [userbot.id, user_id]:
            continue
        try:
...
async def unmuteall(client: Client, message: Message):
    ...
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(
            f"Hey {message.from_user.mention}, 'unmuteall' can only be executed by the group owner {owner_AMBOT}."
        )
        return
    ...
    if callback_query.data == "unmuteall_yes":
        ...
        if not bot.privileges.can_restrict_members:
            await callback_query.message.edit(
                "I don't have permission to restrict members in this group."
            )
            return
        unmuted = 0
...
async def muteall(client: Client, message: Message):
    ...
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(
            f"Hey {message.from_user.mention}, 'muteall' can only be executed by the group owner {owner_AMBOT}."
        )
        return
    confirm_msg = await message.reply(
