...
async def mongo_check_command(client, message: Message):
    if len(message.command) < 2:
        await message.reply(
            "Please provide your MongoDB URL with the command: `/mongochk your_mongo_url`"
        )
        return
    ...
    try:
        ...
        result = (
            f"**MongoDB URL** `{mongo_url}` **is valid**.\n\n**Available Databases:**\n"
        )
...
async def delete_db_command(client, message: Message):
    try:
        ...
        if len(message.command) == 1:
                    ...
                    if number > 0 and number <= len(databases_and_collections):
                        ...
                        try:
                            if col_name:
                                ...
                                await message.reply(
                                    f"**Collection** `{col_name}` **in database** `{db_name}` **has been deleted successfully. 🧹**\n\n**Check Rest databse by: /checkdb, /deldb**"
                                )
                                await ok.delete()
                            else:
                                delete_database(mongo_client, db_name)
                                await message.reply(
                                    f"**Database** `{db_name}` **has been deleted successfully. 🧹**\n\n**Check Rest databse by: /checkdb, /deldb**"
                                )
                                await ok.delete()
                        except Exception as e:
            ...
            if failed:
                await message.reply(
                    f"Some entries could not be deleted or were invalid: {', '.join(failed)} ❌\n\n**Check Rest databse by: /checkdb, /deldb**"
                )
...
async def check_db_command(client, message: Message):
    try:
        ok = await message.reply_text(
            "**Please wait while checking your bot mongodb database...**"
        )
        mongo_client = MongoClient(MONGO_DB_URI, serverSelectionTimeoutMS=5000)
        databases = mongo_client.list_database_names()
...
def backup_old_mongo_data(old_client):
    ...
    for db_name in old_client.list_database_names():
        ...
        for col_name in db.list_collection_names():
            collection = db[col_name]
            backup_data[db_name][col_name] = list(
                collection.find()
            )  # Store all documents
    return backup_data
